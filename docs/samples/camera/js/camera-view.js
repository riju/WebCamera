...
class CameraView extends LitElement {
  ...
  async firstUpdated() {
    ...
    const cabs = this.cameras[0].getCapabilities();
    console.error(cabs["width"].max, cabs["height"].max,
      Math.ceil(visualViewport.width), Math.ceil(visualViewport.height));
    if (cabs.height.max > cabs.width.max) {
